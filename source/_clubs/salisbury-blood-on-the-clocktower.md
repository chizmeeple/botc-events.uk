---
name: Salisbury Blood on the Clocktower
based_in: Salisbury
group_id: salisbury-blood-on-the-clocktower
image: ''
website: ''
meetup: ''
facebook: ''
discord: https://discord.gg/PUTkJXwUJY
bgg: ''
description: >-
  Blood on the Clocktower in Salisbury, aimed at newer players in the area.
  Sessions use the Trouble Brewing script. Limited to 12 players; payment is £5
  by BACS or PayPal (contact organisers via Discord to book and pay).
locations:
  tabletop-tavern:
    name: Tabletop Tavern
    address: 45-49 Catherine St, Salisbury SP1 2DH
    lat: 51.06734495449047
    lng: -1.7936545058252304
    website: https://www.thetabletoptavern.uk/
    parking:
      - onsite: false
        free: true
        name: Brown Street car park
        address: Brown Street, Salisbury SP1 1HE (adjacent to The Red Lion Hotel)
        notes: >-
          Free parking after 6pm.
events:
  recurring:
    - eventname: 'Blood on the Clocktower: Trouble Brewing'
      event_id: first-friday-monthly
      signup: https://discord.gg/PUTkJXwUJY
      cost: "£5"
      startdate: 2026-06-05
      starttime: 1900
      endtime: 2300
      rrule: FREQ=MONTHLY;BYDAY=1FR
      location: tabletop-tavern
---
